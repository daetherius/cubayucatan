...
echo
	...
	echo
		...
		$html->tableHeaders(array(
			...
			$paginator->sort('Disposición', 'layout'),
			$paginator->sort('Imágenes', 'postimg_count'),
		...
		foreach($items as $it){
			...
			echo $html->tableCells(array(array(
				...
				array(
					...
					(isset($it[$_m[0]][mb_strtolower($_m[0]).'img_count']) ? $html->link('Fotos',array('action'=>'images','admin'=>1,$id)):'').
