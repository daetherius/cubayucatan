...
echo
			...
			$html->div('custom_dates block'),
				...
				$html->tag('table'),
					...
					$html->tableCells(array(
						...
						array($html->tag('span','Santa Clara'),						$form->input('santa_clara_arrival',array('class'=>'datepicker','type'=>'text')),		array($form->input('santa_clara_days',array('maxlength'=>3,'value'=>1,'class'=>'days_input')),array('class'=>'days'))),
						array($html->tag('span','Camagüey'),						$form->input('camaguey_arrival',array('class'=>'datepicker','type'=>'text')),			array($form->input('camaguey_days',array('maxlength'=>3,'value'=>1,'class'=>'days_input')),array('class'=>'days'))),
						array($html->tag('span','Baracoa'),							$form->input('baracoa_arrival',array('class'=>'datepicker','type'=>'text')),			array($form->input('baracoa_days',array('maxlength'=>3,'value'=>1,'class'=>'days_input')),array('class'=>'days'))),
						array($html->tag('span','Bayamo'),							$form->input('bayamo_arrival',array('class'=>'datepicker','type'=>'text')),			array($form->input('bayamo_days',array('maxlength'=>3,'value'=>1,'class'=>'days_input')),array('class'=>'days'))),
						array($html->tag('span','Santiago de Cuba'),				$form->input('santiago_de_cuba_arrival',array('class'=>'datepicker','type'=>'text')),	array($form->input('santiago_de_cuba_days',array('maxlength'=>3,'value'=>1,'class'=>'days_input')),array('class'=>'days'))),
						array($html->tag('span','Havana ('.__('retorno',true).')'),	$form->input('havana_arrival2',array('class'=>'datepicker','type'=>'text')),			array($form->input('havana_days2',array('maxlength'=>3,'value'=>1,'class'=>'days_input')),array('class'=>'days'))),
					)),
				'</table>',
				$html->tag('table'),
					$html->tableCells(array(
						...
						array($html->tag('span',__('desayuno',true)),				array($html->tag('span','',array('class'=>'total_days')).' '.__('dias',true).' x 2 x €'.$precio_desayuno.' = '.$html->tag('span','€'.$html->tag('span','',array('id'=>'costo_total_desayuno','class'=>'precio')),'precio'),array('colspan'=>2,'class'=>'subtotales'))),
						
						array($form->input('con_cena',array('type'=>'checkbox','div'=>array('tag'=>'span'))).$html->tag('span',__('cena',true).' ('.__('opcional',true).')','pad'), array($html->tag('span','',array('class'=>'total_days')).' '.__('dias',true).' x 2 x €'.(number_format($precio_cena,1,',','.')).' = '.$html->tag('span','€'.$html->tag('span','',array('id'=>'costo_total_cena','class'=>'precio')),'precio'),array('colspan'=>2,'class'=>'subtotales'))),
					)),
				'</table>',
	...
	$updateSubtotal = 'var inted = $("ReservationHab").get("value").toInt(); if(!isNaN(inted)){ $("num_personas").set("html",inted * 2); $("total_hab").set("html",inted *'.$precio_hab.');$("num_hab").set("html",inted); } ';
	...
	$updateSubtotal.= 'var costo_total_cena = $("ReservationConCena").checked ? total_number_days * 2 * '.$precio_cena.' : 0; $("costo_total_cena").set("html",costo_total_cena.toFixed(2));';
	...
	$updateSubtotal.= 'var big_total = sub_total * $("ReservationHab").get("value").toInt(); $("big_total").set("html",big_total.toFixed(2));';
	
	$moo->addEvent('ReservationHab','keyup',$updateSubtotal);
