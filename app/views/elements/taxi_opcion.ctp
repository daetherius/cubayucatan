...
echo $html->div('taxi_opcion clear'.($item['Pack']['id'] > 4 ? ' yucatan':''));
	...
	if($item['Pack']['id'] > 4){
		echo
			...
			//$form->input('taxi.retorno',array('label'=>__('retorno',true).' '.__('formato_fecha_dmy',true),'class'=>'datepicker')),
			
			$form->input('taxi.adicionales',array(
			...
			$html->div('datos_hotel'),
				$form->input('taxi.nombre_hotel',array('label'=>__('nombre_hotel',true))),
				$form->input('taxi.direccion_hotel',array('label'=>__('direccion_hotel',true))),
			'</div>';
